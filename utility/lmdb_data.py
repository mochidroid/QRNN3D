...
def create_PaviaCentre():
    ...
    datadir = 'data/PaviaCentre_dataset/'
    fns = os.listdir(datadir)
    fns = [fn.split('.')[0]+'.mat' for fn in fns]

    create_lmdb_train(
        datadir, fns, 'data/PaviaCentre64_101', 'pavia',  # your own dataset address
        crop_sizes=None,
        scales=(1,),
        ksizes=(101, 64, 64),
        strides=[(101, 32, 32)],
...
def create_icvl64_31():
    ...
    datadir = 'data/Training_v73/' # your own data address
    # fns = os.listdir(datadir) 
    # fns = [fn.split('.')[0]+'.mat' for fn in fns]

    # ICVL_train.txt を読み込む（拡張子なしのファイル名が想定されている場合）
    with open('ICVL_train.txt') as f:
